...
from ..turing_utils.file_utils import cached_path
from .configuration_turingv3 import TuringV3Config
...
class BertPreTrainedModel(nn.Module):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, TuringV3Config):
            raise ValueError(
                "Parameter config in `{}(config)` should be an instance of class `TuringV3Config`. "
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, state_dict=None, cache_dir=None,
        ...
        config = TuringV3Config.from_json_file(config_file)
        if no_segment:
        ...
        for key in state_dict.keys():
            ...
            if (key == 'bert.embeddings.token_type_embeddings.weight') and (config.type_vocab_size < state_dict[key].shape[0]) and (config.type_vocab_size > 0):
...
class BertModel(BertPreTrainedModel):
    """BERT model ("Bidirectional Embedding Representations from a Transformer").
      ...
      Params:
          config: a TuringV3Config class instance with the configuration to build a new model
      ...
      config = modeling.TuringV3Config(vocab_size_or_config_json_file=32000, hidden_size=768,
...
class BertForPreTraining(BertPreTrainedModel):
    """BERT model with pre-training heads.
      ...
      config = TuringV3Config(vocab_size_or_config_json_file=32000, hidden_size=768,
          num_hidden_layers=12, num_attention_heads=12, intermediate_size=3072)
...
class TuringV3Model(BertModel):
    """TuringV3Model is based on UniLM architecture.
    Taken from the implementation found here:
    https://dev.azure.com/DeepLearningGroup/Alexander/_git/Alexander?path=%2FTNLRv3%2Fnlu-finetune%2Fpytorch_pretrained_bert%2Fmodeling.py&version=GBmaster&line=854&lineEnd=854&lineStartColumn=7&lineEndColumn=16&lineStyle=plain&_a=contents
    """
    def __init__(self, config):
        super(TuringV3Model, self).__init__(config)

class TuringV3ForSequenceClassification(BertForSequenceClassification):
    """TuringV3ForSequenceClassification is similar to BertForSequenceClassification
    except that it uses the TuringV3Model encoder instead of BertModel.
    The V3 encoder is based on the UniLM architecture.
    Taken from the implementation found here:
    https://dev.azure.com/DeepLearningGroup/Alexander/_git/Alexander?path=%2FTNLRv3%2Fnlu-finetune%2Fpytorch_pretrained_bert%2Fmodeling.py&version=GBmaster&line=1159&lineEnd=1159&lineStartColumn=6&lineEndColumn=36&lineStyle=plain&_a=contents
    """
    def __init__(self, config):
        super(TuringV3ForSequenceClassification, self).__init__(config)

class TuringV3ForTokenClassification(BertForTokenClassification):
    """TuringV3ForTokenClassification is similar to BertForTokenClassification
    except that it uses the TuringV3Model encoder instead of BertModel.
    The V3 encoder is based on the UniLM architecture.
    Taken from the implementation found here:
    https://dev.azure.com/DeepLearningGroup/Alexander/_git/Alexander?path=%2FTNLRv3%2Fnlu-finetune%2Fpytorch_pretrained_bert%2Fmodeling.py&version=GBmaster&line=1159&lineEnd=1159&lineStartColumn=6&lineEndColumn=36&lineStyle=plain&_a=contents
    """
    def __init__(self, config):
        super(TuringV3ForTokenClassification, self).__init__(config)