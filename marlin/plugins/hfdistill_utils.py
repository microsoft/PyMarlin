...
class DistillationArguments:
    ...
    loss_types: list = field(default_factory=lambda: ["logits"]) # logits, labels, attentions, hidden_states
    ...
    class DistillHfModule(base):
        ...
        def setup_model(self, automodel_class):
            ...
            if self.args.model_args.model_path is not None:
                ...
                self.teacher_config = MarlinAutoConfig.from_pretrained(self.args.model_args.hf_model)
            ...
            if self.distill_args.student_model_path is not None:
                ...
                self.model = automodel_class.from_pretrained(
                    os.path.join(self.distill_args.student_model_path, self.distill_args.student_model_file),
            ...
            if len(self.distill_args.student_layers) > 0:
                ...
                new_layer_modules = extract_layers(layer_modules, self.distill_args.student_layers)
                getattr(self.model, self.args.model_args.encoder_key).encoder.layer = new_layer_modules
                self.student_config.num_hidden_layers = len(self.distill_args.student_layers)
            self.teacher.eval()
            self.output_hidden = True if 'hidden_states' in self.distill_args.loss_types else False
            self.output_attentions = True if 'attentions' in self.distill_args.loss_types else False
            ...
            for i, k in enumerate(self.distill_args.loss_types):
                ...
                if student_scores is not None and teacher_scores is not None:
                    if k == 'logits':
                        ...
                        total_loss += self.distill_args.loss_weights[i] * representations_loss(
                                        student_scores,
                                        teacher_scores,
