...
class NERProcessor(data_interface.DataProcessor):
    ...
    def __init__(self, args):
        self.args = args
        self.label_map = {label: i for i, label in enumerate(self.args.labels_list)} if self.args.labels_list is not None else None
    ...
    def _create_examples(self, df, tokenizer):
        ...
        for i,record in enumerate(df.to_dict('records')):
            ...
            features.append(self._convert_to_feature(InputExample(guid = i, text_a=text_a, label=label),
             tokenizer))
        return features
        ...
        if self.args.has_labels:
            ...
            for word_idx in word_ids:
                if word_idx is None: # special tokens have None
                    ...
                    label_ids.append(self.label_map[example.label[word_idx]] if self.args.label_all_tokens else self.args.pad_label_id)
...
class NERBaseDataset(Dataset):
    ...
    def _create_tensors(self, feature):     
        ...
        tensor_dict = {'input_ids':input_ids, 'attention_mask': input_mask, 
